...
```{r}
#| echo: false
#| label: fig-cumulative-xG-chart
#| fig-cap: "A graph highlighting the cumulative xG of Ivory Coast through each stage of the tournament"
#| fig-width: 10
#| fig-height: 4
...
The shot maps in @fig-manager-comparison-shotmap offer the clearest immediate illustration of how the team's attacking intent had changed.
...
```{r}
#| label: fig-manager-comparison-shotmap
...
#| cho: false
readRDS("defensive_actions_manager_comparison.rds") |> knitr::kable()
```
...
#| fig-cap: "Passing Heatmap Comparison By Manager"
#| echo: false
#| fig-width: 10
#| fig-height: 4
...
@fig-final-shotmap below illustrates the shot maps of the top five shooters for Ivory Coast.

```{r}
#| label: fig-final-shotmap
...
```{r}
#| label: tbl-combined-goals-and-assists
...
#| echo: false
readRDS("defensive_stats.rds") |> head(5) |> knitr::kable()
...
```{r}
#| label: tbl-progressive-metrics
#| tbl-cap: "Ivory Coast Progressive Metrics"
#| echo: false
readRDS("attacking_metrics.rds") |> head(5) |> knitr::kable()
...
```{r}
#| label: tbl-deserved-winner-tournament
#| tbl-cap: "Tournament's Deserved Winner"
#| echo: false
readRDS("deserved_winner_tournament.rds") |> head(5) |> knitr::kable()
```

The scores revealed that Ivory Coast did in fact deserve to win the tournament despite their poor Group Stage performance as their underlying metrics improved significantly across the tournament. Their transition from the inconsistent group of performers to a more efficient team in the knockout stages is reflected in the high attacking score. All round, they are the most consistent team across the four key attributes.
...
Chang, W., Cheng, J., Allaire, J., Sievert, C., Schloerke, B., Xie, Y., Allen, J., McPherson, J., Dipert, A. and Borges, B. (2024) *shiny: Web Application Framework for R* \[R package\]. Available at: <https://CRAN.R-project.org/package=shiny> (Accessed: 13 May 2026)

R Core Team (2024) *R: A language and environment for statistical computing*. Vienna: R Foundation for Statistical Computing. Available at: <https://www.R-project.org/> (Accessed: 13 May 2026).

StatsBomb (2024) *StatsBomb Open Data* \[online\]. Available at: <https://github.com/statsbomb/open-data> (Accessed: 13 May 2026).

Wickham, H. (2016) *ggplot2: Elegant graphics for data analysis*. New York: Springer. Available at: <https://ggplot2.tidyverse.org> (Accessed: 13 May 2026).
